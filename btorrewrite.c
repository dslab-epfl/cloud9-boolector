...
is_const_one_exp (Btor * btor, BtorExp * exp)
{
  ...
  if (!BTOR_IS_BV_CONST_EXP (BTOR_REAL_ADDR_EXP (exp)))
    return 0;
...
static int is_xor_exp (Btor * btor, BtorExp *exp)
{
  ...
  if (BTOR_REAL_ADDR_EXP (exp)->kind != BTOR_AND_EXP)
    return 0;
  ...
  if (BTOR_REAL_ADDR_EXP (e0_0) != BTOR_REAL_ADDR_EXP (e1_0))
    return 0;
  if (BTOR_REAL_ADDR_EXP (e0_1) != BTOR_REAL_ADDR_EXP (e1_1))
    return 0;
...
static int is_xnor_exp (Btor * btor, BtorExp *exp)
{
  assert (btor != NULL);
  assert (exp != NULL);
  exp = btor_pointer_chase_simplified_exp (btor, exp);
  ...
  if (btor->rewrite_level > 1)
    {
	  ...
	  if (BTOR_IS_INVERTED_EXP (exp))
	    btor_invert_const_3vl (mm, bits_3vl);
...
btor_rewrite_slice_exp (Btor * btor, BtorExp * exp, int upper, int lower)
{
  ...
  /* check if slice and child of concat matches */
    ...
    {
      if (lower == 0 && BTOR_REAL_ADDR_EXP (real_exp->e[1])->len == upper - lower + 1)
        {
          if (BTOR_IS_INVERTED_EXP (exp))
	  ...
	  len = BTOR_REAL_ADDR_EXP (real_exp->e[1])->len;
		...
		goto BTOR_REWRITE_SLICE_EXP_NO_REWRITE;
	      BTOR_INC_REC_RW_CALL (btor);
	      tmp = btor_rewrite_slice_exp (btor, BTOR_COND_INVERT_EXP (exp,
		                            real_exp->e[0]),
		                            upper - len, 0);
...
compute_binary_3vl (Btor * btor, BtorExpKind kind, BtorExp * e0, BtorExp * e1)
{
  ...
  switch (kind)
    {
    case BTOR_AND_EXP:
      result = btor_and_const_3vl (mm, b0, b1);
      break;
    case BTOR_BEQ_EXP:
      result = btor_eq_const_3vl (mm, b0, b1);
      break;
    case BTOR_ADD_EXP:
      result = btor_add_const_3vl (mm, b0, b1);
      break;
    case BTOR_MUL_EXP:
      result = btor_mul_const_3vl (mm, b0, b1);
      break;
    case BTOR_ULT_EXP:
      result = btor_ult_const_3vl (mm, b0, b1);
      break;
    case BTOR_SLL_EXP:
      result = btor_sll_const_3vl (mm, b0, b1);
      break;
    case BTOR_SRL_EXP:
      result = btor_srl_const_3vl (mm, b0, b1);
      break;
    case BTOR_UDIV_EXP:
      result = btor_udiv_const_3vl (mm, b0, b1);
      break;
    case BTOR_UREM_EXP:
      result = btor_urem_const_3vl (mm, b0, b1);
      break;
    default:
...
rewrite_binary_exp (Btor * btor, BtorExpKind kind, BtorExp * e0, BtorExp * e1)
{
  ...
  if (BTOR_IS_BV_CONST_EXP (real_e0) && BTOR_IS_BV_CONST_EXP (real_e1))
	  ...
	  switch (kind)
		...
		if (real_e0->len == 1)
		...
		else if (is_xor_exp (btor, e1))  /* 0 == (a ^ b)  -->  a = b */
		  {
			...
			BTOR_REAL_ADDR_EXP (BTOR_REAL_ADDR_EXP (
			BTOR_REAL_ADDR_EXP (e1)->e[0])->e[0]),
			BTOR_REAL_ADDR_EXP (BTOR_REAL_ADDR_EXP (
			BTOR_REAL_ADDR_EXP (e1)->e[0])->e[1]));
			BTOR_DEC_REC_RW_CALL (btor);
		      }
		  }
		else if (BTOR_IS_INVERTED_EXP (e1) && 
		         real_e1->kind == BTOR_AND_EXP)
		  {      /* 0 == a | b  -->  a == 0 && b == 0 */
		    if (btor->rec_rw_calls < BTOR_REC_RW_BOUND)
		      {
			BTOR_INC_REC_RW_CALL (btor);
		  ...
		  while (pos < real_e0->len)
		    {
		      ...
		      tmp1 = btor_rewrite_slice_exp (btor, BTOR_INVERT_EXP (real_e1->e[0]), 
					             real_e0->len - 1 - pos, 
						     real_e0->len - pos - len);
		      tmp2 = btor_rewrite_slice_exp (btor, BTOR_INVERT_EXP (real_e1->e[1]), 
					     real_e0->len - 1 - pos, real_e0->len - pos - len);
		      if (tmpString[0] == '0')
			{
			  tmp3 = btor_zero_exp (btor, len);
			  BTOR_PUSH_STACK (btor->mm, stack, btor_rewrite_eq_exp (btor, tmp1, tmp3));
			  BTOR_PUSH_STACK (btor->mm, stack, btor_rewrite_eq_exp (btor, tmp2, tmp3));
		      ...
		      else
			{
			  ...
			  BTOR_PUSH_STACK (btor->mm, stack, btor_rewrite_eq_exp (btor, tmp3, tmp4));
			  btor_release_exp (btor, tmp3);
			  btor_release_exp (btor, tmp4);
			}
	  ...
	  switch (kind)
		...
		if (real_e0->len == 1)
		...
		else if (is_xor_exp (btor, e0))   /* (a ^ b) == 0 -->  a = b */
		  {
			...
			result = btor_rewrite_eq_exp (btor, 
			BTOR_REAL_ADDR_EXP (BTOR_REAL_ADDR_EXP (
			BTOR_REAL_ADDR_EXP (e0)->e[0])->e[0]),
			BTOR_REAL_ADDR_EXP (BTOR_REAL_ADDR_EXP (
			BTOR_REAL_ADDR_EXP (e0)->e[0])->e[1]));
			BTOR_DEC_REC_RW_CALL (btor);
		      }
		  }
		else if (BTOR_IS_INVERTED_EXP (e0) && 
			 real_e0->kind == BTOR_AND_EXP)
		  {      /*  a | b == 0  -->  a == 0 && b == 0 */
		    if (btor->rec_rw_calls < BTOR_REC_RW_BOUND)
		      {
			BTOR_INC_REC_RW_CALL (btor);
        ...
        case BTOR_SPECIAL_CONST_ONE_ONES:
          ...
          if (kind == BTOR_AND_EXP || kind == BTOR_BEQ_EXP ||
              kind == BTOR_MUL_EXP || kind == BTOR_UDIV_EXP)
            result = btor_copy_exp (btor, e0);
          break;
        case BTOR_SPECIAL_CONST_ONE:
		  ...
		  while (pos < real_e1->len)
		    {
		      tmpString[0] = bv_const[pos];
		      len = (int) strspn (bv_const + pos, tmpString);
		      tmp1 = btor_rewrite_slice_exp (btor, BTOR_INVERT_EXP (real_e0->e[0]), 
					             real_e1->len - 1 - pos, 
						     real_e1->len - pos - len);
		      tmp2 = btor_rewrite_slice_exp (btor, BTOR_INVERT_EXP (real_e0->e[1]), 
					             real_e1->len - 1 - pos, 
						     real_e1->len - pos - len);
		      if (tmpString[0] == '0')
			{
      ...
      switch (kind)
        ...
        default:
          ...
          break;
           ...
           && real_e0->e[0] == real_e1->e[0] && (real_e0->e[1] == real_e1->e[1]
                                                 || real_e0->e[2] ==
                                                 real_e1->e[2])
               ...
               || kind == BTOR_UDIV_EXP))
    {
      ...
      left = fptr (btor, BTOR_COND_INVERT_EXP (e0, real_e0->e[1]),
                   BTOR_COND_INVERT_EXP (e0, real_e1->e[1]));
      right = fptr (btor, BTOR_COND_INVERT_EXP (e0, real_e0->e[2]),
...
is_always_unequal (Btor * btor, BtorExp * e0, BtorExp * e1)
{
  ...
  if (e0 == BTOR_INVERT_EXP (e1))
    return 1;

  if (BTOR_IS_BV_CONST_EXP (real_e0) && BTOR_IS_BV_CONST_EXP (real_e1) && e0 != e1)
    return 1;
...
static void
normalize_binary_comm_ass_exp (Btor * btor, BtorExp * e0, BtorExp * e1,
                               BtorExp ** e0_norm, BtorExp ** e1_norm,
                               BtorExp * (*fptr) (Btor *, BtorExp *, BtorExp *),
                               BtorExpKind kind)
{
  BtorMemMgr *mm;
...
static void
normalize_adds_exp (Btor * btor, BtorExp * e0, BtorExp * e1, BtorExp ** e0_norm,
                    BtorExp ** e1_norm)
{
  assert (btor != NULL);
  assert (e0 != NULL);
  ...
  normalize_binary_comm_ass_exp (btor, e0, e1, e0_norm, e1_norm, btor_rewrite_add_exp,
                                 BTOR_ADD_EXP);
}

static void
normalize_muls_exp (Btor * btor, BtorExp * e0, BtorExp * e1, BtorExp ** e0_norm,
                    BtorExp ** e1_norm)
{
  assert (btor != NULL);
  assert (e0 != NULL);
  ...
  normalize_binary_comm_ass_exp (btor, e0, e1, e0_norm, e1_norm, btor_rewrite_mul_exp,
                                 BTOR_MUL_EXP);
}
...
static int
find_and_contradiction_exp (Btor * btor, BtorExp *exp, BtorExp *e0, BtorExp *e1, int *calls)
{
  assert (btor != NULL);
  assert (exp != NULL);
  assert (e0 != NULL);
  ...
  if (*calls >= BTOR_FIND_AND_EXP_CONTRADICTION_LIMIT)
    return 0;

  if (!BTOR_IS_INVERTED_EXP (exp) && exp->kind == BTOR_AND_EXP)
    {
      if (exp->e[0] == BTOR_INVERT_EXP (e0) || 
	  exp->e[0] == BTOR_INVERT_EXP (e1) ||
  ...
  if (real_e0->kind == BTOR_AND_EXP && real_e1->kind == BTOR_AND_EXP)
    {
      if (!BTOR_IS_INVERTED_EXP (e0) && !BTOR_IS_INVERTED_EXP (e1))
		...
		goto BTOR_REWRITE_AND_EXP_NO_REWRITE;
	      BTOR_INC_REC_RW_CALL (btor);
              result = btor_rewrite_eq_exp (btor, BTOR_REAL_ADDR_EXP (real_e0->e[0]),
      ...
      if (!BTOR_IS_INVERTED_EXP (e0))
        {
		  ...
		  BTOR_INC_REC_RW_CALL (btor);
                  ...
                  result = btor_rewrite_and_exp (btor, BTOR_INVERT_EXP (e0->e[0]), tmp1);
		  BTOR_DEC_REC_RW_CALL (btor);
                  btor_release_exp (btor, tmp1);
                  return result;
                }
      ...
      if (!BTOR_IS_INVERTED_EXP (e1))
        {
		  ...
		  BTOR_INC_REC_RW_CALL (btor);
                  ...
                  result = btor_rewrite_and_exp (btor, BTOR_INVERT_EXP (e1->e[0]), tmp1);
		  BTOR_DEC_REC_RW_CALL (btor);
                  btor_release_exp (btor, tmp1);
                  return result;
                }
	  ...
	  if (e0->kind == BTOR_AND_EXP && e1->kind == BTOR_AND_EXP)
	    {
		  ...
		  if (BTOR_IS_INVERTED_EXP (e0->e[0]) == 
			...
			goto BTOR_REWRITE_EQ_EXP_NO_REWRITE;
		      BTOR_INC_REC_RW_CALL (btor);
		      result = btor_rewrite_eq_exp (btor, e0->e[0],
			                            BTOR_INVERT_EXP (e0->e[1]));
		      BTOR_DEC_REC_RW_CALL (btor);
		      return result;
		    }
              ...
              if (BTOR_IS_INVERTED_EXP (e0))
                {
                  tmp1 =
                    btor_rewrite_eq_exp (btor,
                                         BTOR_INVERT_EXP (real_e0->e[2]), e1);
                  result =
                    btor_rewrite_and_exp (btor, BTOR_INVERT_EXP (real_e0->e[0]), tmp1);
              ...
              if (BTOR_IS_INVERTED_EXP (e0))
                {
                  tmp1 =
                    btor_rewrite_eq_exp (btor,
                                         BTOR_INVERT_EXP (real_e0->e[1]), e1);
              ...
              else
                {
                  ...
                  result =
                    btor_or_exp (btor, BTOR_INVERT_EXP (real_e0->e[0]), tmp1);
                }
              btor_release_exp (btor, tmp1);
	      BTOR_DEC_REC_RW_CALL (btor);
              return result;
              ...
              if (BTOR_IS_INVERTED_EXP (e1))
                {
                  tmp1 =
                    btor_rewrite_eq_exp (btor,
                                         BTOR_INVERT_EXP (real_e1->e[2]), e0);
                  result =
                    btor_rewrite_and_exp (btor, BTOR_INVERT_EXP (real_e1->e[0]), tmp1);
              ...
              if (BTOR_IS_INVERTED_EXP (e1))
                {
                  tmp1 =
                    btor_rewrite_eq_exp (btor,
                                         BTOR_INVERT_EXP (real_e1->e[1]), e0);
              ...
              else
                {
                  ...
                  result =
                    btor_or_exp (btor, BTOR_INVERT_EXP (real_e1->e[0]), tmp1);
                }
              btor_release_exp (btor, tmp1);
	      BTOR_DEC_REC_RW_CALL (btor);
              return result;
...
btor_rewrite_udiv_exp (Btor * btor, BtorExp * e0, BtorExp * e1)
{
  ...
  if (BTOR_REAL_ADDR_EXP (e0)->len == 1)
	...
	goto BTOR_REWRITE_UDIV_EXP_NO_REWRITE;
      BTOR_INC_REC_RW_CALL (btor);
      result = BTOR_INVERT_EXP (btor_rewrite_and_exp (btor, BTOR_INVERT_EXP (e0), e1));
      BTOR_DEC_REC_RW_CALL (btor);
      return result;
    }
...
btor_rewrite_concat_exp (Btor * btor, BtorExp * e0, BtorExp * e1)
{
  ...
  if (BTOR_IS_BV_CONST_EXP (BTOR_REAL_ADDR_EXP (e1))
      ...
      && BTOR_IS_BV_CONST_EXP (BTOR_REAL_ADDR_EXP (BTOR_REAL_ADDR_EXP (e0)->e[1])))
    {
      if (btor->rec_rw_calls >= BTOR_REC_RW_BOUND)
	goto BTOR_REWRITE_CONCAT_EXP_NO_REWRITE;
      ...
      if (!BTOR_IS_INVERTED_EXP (e0))
        ...
        {
          temp =
            btor_rewrite_concat_exp (btor,
                             BTOR_INVERT_EXP (BTOR_REAL_ADDR_EXP (e0)->e[1]),
                             e1);
          result =
            btor_rewrite_concat_exp (btor,
                             BTOR_INVERT_EXP (BTOR_REAL_ADDR_EXP (e0)->e[0]),
                             temp);
          btor_release_exp (btor, temp);
        }
      BTOR_DEC_REC_RW_CALL (btor);
...
btor_rewrite_read_exp (Btor * btor, BtorExp * e_array, BtorExp * e_index)
{
  ...
  if (BTOR_IS_WRITE_EXP (e_array))
    {
      ...
      if (e_index == write_index)
        return btor_copy_exp (btor, e_array->e[2]);
  ...
  if (result->bits == NULL)
    result->bits = btor_x_const_3vl (mm, e_array->len);
#endif
...
BtorExp *
btor_rewrite_write_exp (Btor * btor, BtorExp * e_array, BtorExp * e_index,
                        BtorExp * e_value)
{
...
BtorExp *
btor_rewrite_cond_exp (Btor * btor, BtorExp * e_cond, BtorExp * e_if,
                       BtorExp * e_else)
{
      ...
      if (BTOR_REAL_ADDR_EXP (e_if)->e[0] == e_cond)
        {
          ...
          BTOR_INC_REC_RW_CALL (btor);
          result = btor_rewrite_cond_exp (btor, e_cond, BTOR_COND_INVERT_EXP (e_if, BTOR_REAL_ADDR_EXP (e_if)->e[1]), e_else);
          BTOR_DEC_REC_RW_CALL (btor);
          return result;
        }
  ...
  if (BTOR_IS_ARRAY_OR_BV_COND_EXP (BTOR_REAL_ADDR_EXP (e_else)))
    {
      if (BTOR_REAL_ADDR_EXP (e_else)->e[0] == e_cond)
        {
          if (btor->rec_rw_calls >= BTOR_REC_RW_BOUND)
            goto BTOR_REWRITE_COND_EXP_NO_REWRITE;
          BTOR_INC_REC_RW_CALL (btor);
          result = btor_rewrite_cond_exp (btor, e_cond, e_if, BTOR_COND_INVERT_EXP (e_else, BTOR_REAL_ADDR_EXP (e_else)->e[2]));
          BTOR_DEC_REC_RW_CALL (btor);
          return result;
        }
	  ...
	  BTOR_INC_REC_RW_CALL (btor);
          tmp4 =
            btor_rewrite_and_exp (btor, BTOR_INVERT_EXP (e_cond), BTOR_INVERT_EXP (tmp1));
  ...
  if (kind == BTOR_BCOND_EXP)
    {
	  ...
	  BTOR_INC_REC_RW_CALL (btor);
          tmp1 =
            btor_uext_exp (btor, BTOR_INVERT_EXP (e_cond),
                           BTOR_REAL_ADDR_EXP (e_if)->len - 1);
